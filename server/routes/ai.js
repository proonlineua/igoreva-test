...
НЕ включай полные должностные инструкции — для них отдельный инструмент.

ВАЖНО: Раздел 3.1 — схему подчинения — ГЕНЕРИРУЙ КАК HTML (не Markdown). Используй точно эти CSS-классы.
Каждый HTML-блок — на одной строке без переносов внутри тегов.

### 3.1 Схема организационной структуры

<div class="chart-block"><div class="tree"><div class="node n-owner">[Директор/Владелец]<small>[ФИО или «Владелец»]</small></div><div class="connector"></div><div class="hbar"></div><div class="groups"><div class="group"><div class="col-label">Управление</div><div class="stack"><div class="node n-mgmt">[Старший мастер]<small>[N] чел.</small></div></div></div><div class="group"><div class="col-label">Сервис</div><div class="stack"><div class="node n-prod">[Мастера]<small>[N] чел.</small></div></div></div><div class="group"><div class="col-label">Фронт</div><div class="stack"><div class="node n-front">[Администратор]<small>[N] чел.</small></div></div></div></div></div></div>
...
| Старший мастер | [N] | Директор | [заполнить 2–3 фразы] | [мин–макс] |
| Мастер | [N] | Ст. мастер / Директор | [заполнить 2–3 фразы] | [мин–макс] |
| Администратор | [N] | Директор | [заполнить 2–3 фразы] | [мин–макс] |
...
- [критерий 4 — специфичный для данного салона]

<div class="oc-callout"><b>Доплата:</b> +[сумма] ${cur}/мес → итоговая зарплата <b>[итог] ${cur}</b></div>
...
- Что делать с неработающими сотрудниками: чёткий алгоритм`,
    ...
    job_desc: `
ИСПОЛЬЗУЙ СТРОГО ЭТУ СТРУКТУРУ ДЛЯ РАЗДЕЛА 3. НЕ изменяй заголовки, не добавляй и не убирай подразделы.
Специальность, тип оплаты и % / сумму берёт СТРОГО из "ДОПОЛНИТЕЛЬНЫЕ ДАННЫЕ".
Всё содержимое — специфично для выбранной специальности, никаких обобщённых фраз "для мастера".
ЭТОТ ДОКУМЕНТ ЗАМЕНЯЕТ ТРИ ОТДЕЛЬНЫХ: должностную инструкцию + KPI + систему мотивации. Разделы 3.4 и 3.8 должны быть детальными и полными.

### 3.1 Общая информация

| Параметр | Значение |
|----------|----------|
| Должность | [специальность из ответа — точно как указано] |
| Подчиняется | [директор / управляющий / старший мастер — из оргструктуры] |
| Рабочий график | [из данных салона: дни и часы] |
| Тип оплаты | [тип из ответа] |
| Размер оплаты | [% или сумма из ответа] |
| Зарплата при 70% загрузке | [расчёт в ${cur}] |
| Зарплата при 90% загрузке | [расчёт в ${cur}] |
| Максимально возможная | [расчёт в ${cur}] |

### 3.2 Профессиональные требования к [специальность]

**Обязательно (без этого не берём):**
- [требование 1 — специфично для данной специальности]
- [требование 2]
- [требование 3]
- [требование 4]
- [требование 5]

**Желательно (конкурентное преимущество):**
- [навык 1]
- [навык 2]

### 3.3 Рабочий день по часам

| Время | Обязательное действие | Стандарт / Как проверить |
|-------|-----------------------|--------------------------|
| [начало смены − 15 мин] | Подготовка рабочего места | [чек-лист открытия] |
| [рабочие часы — конкретные временные слоты под специальность] | [действие] | [стандарт] |
| [между клиентами] | [действие] | [стандарт] |
| [конец смены] | Закрытие рабочего места | [чек-лист закрытия] |

### 3.4 KPI и расчёт оплаты

| Показатель | Формула расчёта | Минимум | Цель | Влияние на оплату |
|-----------|----------------|---------|------|------------------|
| Личная выручка | сумма услуг за месяц | [X] ${cur} | [Y] ${cur} | [база] |
| Загрузка | визиты / слоты × 100% | [X]% | [Y]% | [+/− влияние] |
| Средний чек | выручка / кол-во клиентов | [X] ${cur} | [Y] ${cur} | [+/− влияние] |
| Возвращаемость | повторные / всего × 100% | [X]% | [Y]% | [+/− влияние] |
| Допродажи | сумма доп. услуг / средств | [X] ${cur} | [Y] ${cur} | [+/− влияние] |

**Пример расчёта зарплаты за месяц:**
Выручка [пример] ${cur} × [%] = [сумма] ${cur} базовая
+ Бонус за цель по загрузке: +[сумма] ${cur}
+ Бонус за допродажи: +[сумма] ${cur}
= Итого: [итог] ${cur}

### 3.5 Запрещено — основание для выговора / увольнения

1. [запрет 1 — специфичный для специальности, конкретный]
2. [запрет 2]
3. [запрет 3]
4. [запрет 4]
5. [запрет 5]

### 3.6 Испытательный срок (3 месяца)

| Период | Что должен уметь / показать | Как проверяем |
|--------|---------------------------|---------------|
| Месяц 1 | [конкретные навыки для специальности] | [метод проверки] |
| Месяц 2 | [KPI: выручка ≥ X ${cur}, загрузка ≥ Y%] | [замер] |
| Месяц 3 | [выход на целевые показатели] | [финальная оценка] |

**Условие успешного прохождения:** [метрики + поведение]

### 3.7 Карьерный рост

| Уровень | Условия перехода | Доплата |
|---------|-----------------|---------|
| [текущий уровень из ответа] | — | текущая ставка |
| Следующий уровень | [конкретные условия: стаж + выручка + оценки] | +[сумма] ${cur} |
| Старший / Ведущий | [условия] | +[сумма] ${cur} |

### 3.8 Система мотивации и грейды

| Грейд | Условие входа | Ставка | Бонусы и привилегии |
|-------|-------------|--------|---------------------|
| Стажёр | 0–3 мес испытательного срока | [X]% или [Y] ${cur} фикс | [что получает] |
| [Специальность] — база | стаж ≥ [X] мес, выручка ≥ [Y] ${cur} | [%] | [бонусы] |
| [Специальность] — опыт | стаж ≥ [X], выручка ≥ [Y] ${cur}, retention ≥ [Z]% | [%] | [бонусы] |
| Старший / Ведущий | стаж ≥ 3 лет, выручка ≥ [Y] ${cur}, менторство | [%] + [доплата] ${cur} | [привилегии] |

**Бонусная система:**
- Перевыполнение плана на 10%+: +[сумма] ${cur}
- Новый клиент по рекомендации: +[сумма] ${cur} за каждого
- Retention ≥ 80% по итогам месяца: +[сумма] ${cur}
- Лучший мастер месяца (по выручке + оценкам): +[сумма] ${cur}

**Что снижает выплату (не штраф, а корректировка):**
- [нарушение стандарта 1]: −[сумма] ${cur} или −[X]%
- [нарушение стандарта 2]: −[сумма] ${cur}
- Опоздание на смену: −[сумма] ${cur} за каждый случай`,
