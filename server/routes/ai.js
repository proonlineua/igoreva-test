...
## 1. Текущая ситуация
На основе аудита — что есть, чего нет прямо сейчас, и сколько денег это стоит.
Посчитай конкретные потери в ${cur}/мес: недополученная выручка от отсутствия этого инструмента.
Пример логики: "Нет скриптов → конверсия ~40% вместо 65% → из ${salonData?.revenue || 'текущей выручки'} ${cur} теряется X ${cur}/мес".

## 2. Как должно быть
Эталонное состояние для салона ${salonData?.masters || ''} мастеров в ${country}.
— Конкретные целевые метрики (не "улучшить", а "конверсия ≥ 65%", "retention ≥ 55%")
— Потенциал роста выручки при внедрении этого инструмента: +X ${cur}/мес (покажи расчёт)
— Сравнение с нормой рынка ${country} ${new Date().getFullYear()} года

## 3. [РАБОЧИЙ ДОКУМЕНТ — готов к использованию с первого дня]
Здесь сам инструмент: скрипт / чек-лист / KPI-карточки / расчёт / регламент / план.
Конкретный, под этот салон, никаких шаблонных фраз.
Используй Markdown-таблицы для KPI, расчётов, матриц, временных рамок.

## 4. Что делать руководителю
Конкретный план действий:
— Неделя 1: [первые шаги, что внедрить немедленно]
— Недели 2–4: [что настроить, кого обучить]
— Месяц 2–3: [что проверить, как скорректировать]
— Ожидаемый результат: +X ${cur}/мес через Y месяцев (конкретная сумма)

ТРЕБОВАНИЯ:
- Все цифры в ${cur} — НЕ абстрактные проценты без сумм. Всегда переводи % в деньги.
- Расчёт потенциала ОБЯЗАТЕЛЕН: покажи сколько зарабатывает салон сейчас и сколько будет после внедрения
- Таблицы Markdown для KPI, расчётов, матриц — не перечисления строчкой
- Расшифровывай термины при первом упоминании: ФОТ, BEP, CAC, LTV, ARPU
- Ноль воды и общих слов. Каждое предложение — конкретное действие или конкретная цифра.
