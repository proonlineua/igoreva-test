...
НЕ включай полные должностные инструкции — для них есть отдельный инструмент "Должностная инструкция".
Оргструктура — это КТО кому подчиняется и КТО за что отвечает, не детали работы каждого.

1. Схема структуры (ASCII-диаграмма):
   Директор → [Старший мастер → Мастера] + [Администратор]
   Показать уровни подчинения и количество человек на каждом уровне

2. Таблица ролей: | Роль | Кол-во | Кому подчиняется | Зона ответственности (2–3 фразы) | Зарплатная вилка ${cur} |

3. Назначение старшего мастера:
   - Критерии: стаж ≥ X лет, загрузка ≥ Y%, рейтинг ≥ Z
   - Доплата: +3 000–5 000 ${cur}/мес
   - Список обязанностей старшего мастера (5–7 пунктов)

4. RACI-матрица — только ключевые процессы (не более 15 строк):
   | Процесс | Директор | Старший мастер | Мастер | Администратор |
   Блоки: Найм и увольнение / Обучение и качество / Операционные решения
   Статусы: R (выполняет), A (отвечает), C (консультирует), I (информируют)

5. Лимиты самостоятельных решений по должностям:
   | Должность | До X ${cur} — сам | X–Y ${cur} — согласует | > Y ${cur} — директор |`,
